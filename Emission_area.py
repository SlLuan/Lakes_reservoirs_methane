...
from matplotlib.ticker import MaxNLocator  # 新增：用于限制刻度数量
...
def get_levels(data_masked, method='auto', num_levels=11, manual_levels=None):
    ...
    参数:
      data_masked: 掩膜处理后的数据（numpy 数组）
      method: 分级方式，可选 'auto'（自动化分），'percentile'（百分比划分），'manual'（全手动）
      num_levels: 等级数量（适用于 'auto' 和 'percentile' 方式）
      manual_levels: 当 method 为 'manual' 时，用户提供的级数列表
    返回:
      levels: 用于颜色映射的分级数组
    """
    if method == 'auto':
        ...
        if manual_levels is None:
            raise ValueError("请提供 manual_levels 列表，当 method 设置为 'manual' 时。")
        levels = np.array(manual_levels)
    else:
        raise ValueError("未知的 method 参数，请选择 'auto', 'percentile' 或 'manual'.")
    return levels
...
def plot_LCT_flux_map(data_masked, transform, levels, legend_label, caption, ax):
    ...
    # 新增：限制颜色条刻度数，避免刻度标签重叠（此处设置 nbins=5，可根据需要调整）
    # cbar.ax.xaxis.set_major_locator(MaxNLocator(nbins=6))
    ticks = cbar.get_ticks()
    cbar.set_ticks(ticks[::1])  # 间隔2个显示一个
...
# tif_file = '/mnt/private1/luansl/0Results_for_paper/CH4_from_water/Data/LAK_RSV/CNGHGv1.0_CH4_AFOLU_LAK_RSV_mean_2000_2023Mg.tif'
# tif_file = '/mnt/private1/luansl/0Results_for_paper/CH4_from_water/Data/LAK/CNGHGv1.0_CH4_AFOLU_LAK_mean_2000_2023Mg.tif'
tif_file = '/mnt/private1/luansl/0Results_for_paper/CH4_from_water/Data/RSV/CNGHGv1.0_CH4_AFOLU_RSV_mean_2000_2023Mg.tif'
...
# manual_levels = [0, 10, 15, 20, 25, 40, 60, 80, 100, 200, 300, 400, 500, 800, 1000, 1500, 2000, 2500, 4000, 5000, 5500, 6000, 7000]  #a
# manual_levels = [25, 30, 35, 40, 50, 60, 100, 150, 200, 400, 600, 1500, 2000, 4000, 5000, 5500, 6000, 7000]  #b
manual_levels = [16, 30, 50, 65, 100, 130, 180, 300, 450, 1000, 2000, 4500]  #c
# manual_levels = [np.nanmin(data_masked),0.05, 0.1, 0.15, 0.2, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5]
...
legend_label = 'CH$_4$ Emissions ($\mathrm{Mg}$ CH$_4$/yr)'
caption = '(c)'
...
plt.savefig(f'/mnt/private1/luansl/0Results_for_paper/CH4_from_water/Figures/Figure4/Figure{caption}.png', dpi=350)
